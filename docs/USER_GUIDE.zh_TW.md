...
程式每 30 秒保存一份復原工作區，內容包含內嵌 ActionSpec、主要與比較 Take、A／B 顯示狀態及播放 tick。若上次異常關閉，按「復原」即可回到相同審核位置；確認後立刻儲存為正常 `.adproject` 並重新匯出 JSON。舊版只含 ActionSpec 的復原檔仍可開啟。自動存檔不是正式版本管理的替代品。
...
重新啟動後按「復原」，續接 30 秒工作區自動存檔所保存的 Take 配對、A／B 狀態與播放頭；檢查內容後立即存成正常專案並重新匯出。
