...
保存目前動作、專案資料夾、素材索引與可復原資料。匯入外部素材前必須先儲存專案，因為素材需要穩定的專案相對路徑。
...
3. 若攻擊框關閉前沒有回報，Runtime 自動設為 `miss`。
