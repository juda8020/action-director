...
Every 30 seconds, the editor saves a recovery workspace containing the embedded ActionSpec, primary and comparison Takes, A/B visibility, and playhead tick. After an abnormal exit, Recover resumes that exact review position. If the newest recovery workspace is damaged, Recover automatically tries the previous valid 30-second backup and tells you that it did so; the damaged file is not rewritten. Inspect the recovered state, then immediately save and export to normal project paths. Recovery is not a replacement for source control. Older action-only recovery files remain readable.
...
- **Abnormal exit:** choose Recover to resume the 30-second workspace autosave at its saved Take pair, A/B state, and playhead. If the latest file is damaged, Recover tries the previous valid backup automatically. Then save and export the result to normal paths.
