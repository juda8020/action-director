...
- 加入程式內「展示模式」：隱藏素材庫、Inspector 與編排工具，保留雙 Take
  預演、語意差異摘要與時間軸；可直接用於團隊審核、教學與公開截圖。
- 2D 與 3D 範例都提供可直接比較的 Take A／Take B，摘要會顯示接觸時間差
  與兩版有效窗口，不再只顯示抽象的總長度差。

### 變更

- 重寫 Take 語意差異演算法：比較每個 tick 的實際事件狀態，不再因事件的
  結束時間不同而錯誤回報「首次差異 tick 0」。2D 範例目前在 tick 16
  首次分歧，Take B 於 tick 18 接觸，比 Take A 快 4 ticks。
- 3D FBX 衝撞範例改為 Take A／Take B 同畫面比較；Take B 提早 5 ticks
  接觸。攝影機會完整容納角色，目標換成一致的 3D 訓練假人，Hitbox 改為
  貼近胸前的半透明線框，而非不透明紅色方塊。
- 頂部工具列依「專案／歷程／播放／比較」分組，避免所有按鈕視覺權重相同。
- Inspector 預設以可讀摘要顯示動畫、窗口、位移、Hitbox 與手感設定；原始
  Payload JSON 收進可展開的進階區，仍保留完整資料編輯能力。
...
- 重製兩張由程式展示模式直接產生的公開產品截圖：2D 停在 Take B 已進入
  Hitbox、Take A 仍在前搖的第 18 tick；3D 停在 Take B 接觸的第 35 tick。
...
- 測試總數增加至 31 項，新增內附 2D spritesheet 的透明度、
  ...
  Undo／Redo，以及切換 Take 時的 72-tick 時間上限重綁。
