class_name ActionActorAdapter3D
extends Node
...
func _on_motion_requested(delta: Variant, space: String) -> void:
	if actor == null or not delta is Array or delta.size() < 3:
		return
	var motion := Vector3(float(delta[0]), float(delta[1]), float(delta[2]))
	actor.position += actor.basis * motion if space == "local" else motion
