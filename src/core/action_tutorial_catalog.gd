...
const CONTENT := {
	"en": {
		...
		"workspace": {"title": "Projects, source files, and recovery", "summary": "Keep editable workspaces, portable action specs, and imported assets organized safely.", "time": "5 min", "action": "none", "steps": [
			...
			["Recover deliberately", "Recover resumes the 30-second workspace autosave with its Take pair, A/B state, and playhead. If the latest file is damaged, it automatically tries the previous valid backup and tells you. Inspect the result, then immediately save and export to normal paths; autosave does not replace Git."],
		]},
		...
		"troubleshooting": {"title": "Import and recovery troubleshooting", "summary": "Fix the failures most likely to interrupt a first project.", "time": "4 min", "action": "none", "steps": [
			...
			["Crash recovery", "Choose Recover to resume the 30-second workspace autosave at its saved Take pair, A/B state, and playhead. If the latest file is damaged, Recover tries the previous valid backup automatically. Then save/export it to a normal project path."],
		]},
	...
	"zh_TW": {
		...
		"workspace": {"title": "專案、正式檔案與復原", "summary": "正確管理工作區、ActionSpec、外部素材與異常復原。", "time": "5 分鐘", "action": "none", "steps": [
			...
			["正確使用復原", "「復原」會續接每 30 秒建立的工作區自動存檔，還原 Take 配對、A／B 狀態與播放頭。若最新檔案損壞，程式會自動改讀上一份有效備份並提示。確認後立刻保存並匯出到正常路徑；自動存檔不能取代 Git。"],
		]},
		...
		"troubleshooting": {"title": "匯入與復原疑難排解", "summary": "處理最容易中斷第一個專案的問題。", "time": "4 分鐘", "action": "none", "steps": [
			...
			["崩潰復原", "按「復原」續接每 30 秒的工作區自動存檔與審核位置；若最新檔案損壞，程式會自動嘗試上一份有效備份。接著立刻存到正常專案路徑。"],
		]},
	...
	"ja": {
		...
		"workspace": {"title": "Project、正式Source、復元", "summary": "Workspace、ActionSpec、外部Asset、異常終了の復元を安全に管理します。", "time": "5分", "action": "none", "steps": [
			...
			["復元を確定保存", "復元は30秒ごとのWorkspace AutosaveをTakeペア、A/B状態、再生ヘッドと共に再開します。最新Fileが破損している場合は直前の有効なBackupを自動で試して通知します。確認後すぐ通常Pathへ保存・書出し、Gitの代用にはしません。"],
		]},
		...
		"troubleshooting": {"title": "読込と復元のトラブル対処", "summary": "最初のプロジェクトを止めやすい問題を解決します。", "time": "4分", "action": "none", "steps": [
			...
			["クラッシュ復元", "「復元」で30秒ごとのWorkspace Autosaveと保存済みレビュー位置を再開します。最新Fileが破損している場合は直前の有効なBackupを自動で試します。その後すぐ通常のプロジェクトパスへ保存／書き出します。"],
		]},
	...
	"ko": {
		...
		"workspace": {"title": "프로젝트, 공식 원본, 복구", "summary": "작업공간, ActionSpec, 외부 에셋과 비정상 종료 복구를 안전하게 관리합니다.", "time": "5분", "action": "none", "steps": [
			...
			["복구 후 정상 저장", "복구는 30초 작업공간 autosave를 Take 쌍, A/B 상태, 재생 헤드와 함께 이어서 엽니다. 최신 파일이 손상되었으면 이전 유효 백업을 자동으로 시도하고 알립니다. 확인 후 즉시 정상 위치로 저장·내보내며 Git 대용으로 사용하지 않습니다."],
		]},
		...
		"troubleshooting": {"title": "가져오기와 복구 문제 해결", "summary": "첫 프로젝트를 막기 쉬운 문제를 해결합니다.", "time": "4분", "action": "none", "steps": [
			...
			["충돌 복구", "‘복구’로 30초 작업공간 자동 저장과 저장된 검토 위치를 이어서 엽니다. 최신 파일이 손상되었으면 이전 유효 백업을 자동으로 시도합니다. 이후 정상 프로젝트 경로로 즉시 저장/내보냅니다."],
		]},
